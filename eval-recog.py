            ...
            for im_fn in im_fn_list:
                ...
                if boxes is not None:
                    ...
                    with open(res_file, 'w') as f:
                        count = 0
                        for box in boxes:
                            #tess_image = im[int(box[1]):int(box[5]), int(box[0]):int(box[2])]
                            tess_image = im[int(box[0][1]):int(box[2][1]), int(box[0][0]):int(box[2][1])]
                            # if (tess_image != []):
                            #     cv2.imwrite("image_"+ "count" + ".jpg", tess_image)
                            ...
                            entity_block = ET.Element("block_data", block_number = str(block_count), StartX=str(int(box[0][0])), StartY=str(int(box[0][1])), EndX=str(int(box[2][1])), EndY=str(int(box[2][1])))
                ...
                try:
                    ...
                    xml_path = os.path.join(os.path.join(FLAGS.output_dir), xmlfilename)
